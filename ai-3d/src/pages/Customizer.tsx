...
import state,{type StateType} from "../store"; 
...
const Customizer = () => {
  ...
  const handleDecals = (type: string, result: string) => {
    const decalType = DecalTypes[type as "logo" | "full"];
 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (state as Record<string, any>)[decalType.stateProperty as keyof StateType] = result;

    if (!activeFilterTab[decalType.filterTab as "logoShirt" | "stylishShirt"]) {
      handleActiveFilterTab(
        decalType.filterTab as "logoShirt" | "stylishShirt"
      );
    }
  };
          ...
          >
            {FilterTabs.map((tab) => (
              <Tab
                ...
                handleClick={() =>
                  handleActiveFilterTab(
                    tab.name as "logoShirt" | "stylishShirt"
                  )
                }
              />
            ))}
